...
def choice_A(code):
    ...
    for i in range(7,len(dfc)-1):
        ...
        if dfc['CLOSE'][i] < dfc['CLOSE'][i-1] : # 下降趋势
            ...
            if dfc['keep_decline_days'][i] >= 3 and k1['bar'] >= 2*average_bar and k2['bar'] >= 2*average_bar\
                    and dfc['CLOSE'][i+1] > dfc['HIGH'][i] \
                    and dfc['OPEN'][i+1] >= dfc['CLOSE'][i]: # one_soldiers（反转看涨）:
                dfc['one_soldiers'][i] = 1
    ...
    df_res = dfc[dfc['one_soldiers']==1]
    # 结果汇总
    a = len(df_res) # one_soldiers天数
    ...
    # print(res)
    writetocsv([res],'白色一兵result.csv')
    print(df_res)
    return df_res

def main(codes):
    df_list = []
    header = ['code','one_soldiers天数','平均上涨幅度','最大上涨幅度','胜率','最大涨幅日期','每日涨幅','每日总幅度','最近出现日期']
    writetocsv([header],'白色一兵result.csv')
    for code in codes[:3]:
        save_path ='D:\\Desktop\\python量化\\白色一兵\\' + code + 'one_soldiers.csv'
        try:
